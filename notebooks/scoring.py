...
def score(
    ...
    rle_column_name: str = "rle",
    tolerance: float = 0.0,
    resize_fraction: float = 1.0,
) -> float:
    """Mean Surface Dice over collections of 2D data.
    ...
    >>> score(solution, submission, 'id', 'rle', 5.0)
    1.0
...
def voting_resize(mask, new_shape):
    interpolated_mask = resize(
        mask, new_shape, order=1, preserve_range=True
...
def compute_surface_distances(mask_gt, mask_pred, spacing_mm):
    ...
    if num_dims == 2:
        ...
        raise ValueError("Only 2D masks are supported, not " "{}D.".format(num_dims))
...
if __name__ == "__main__":
    ...
    res = score(solution, submission, "id", "rle", 0.0)
    print(res)
